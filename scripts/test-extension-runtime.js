...
function createState(entries) {
  return {
    ...
    structureMode: false,
    targetLanguagePages: false,
    hideTextUntilTranslated: false,
    duolingoAutoContinue: false,
    duolingoTypeAnswers: false,
    duolingoCopyPhrase: false,
    duolingoLowercaseBank: false,
    duolingoDecoyWords: false,
  ...
  await page.evaluate(() => {
    window.__learnedWordReplacerTestConfig.Translator = {
      ...
      create: async (options) => {
        ...
        const direction = {
          sourceLanguage: options ? options.sourceLanguage : "",
          targetLanguage: options ? options.targetLanguage : ""
        };
        const instance = {
          translate: (text) => window.__testTranslatorTranslate(text, direction)
        };
...
function createBatchAwareTranslate(translator) {
  // Mocks that only care about one direction can keep ignoring the second
  // argument; target-language page tests use it to answer target→English.
  return async (text, direction = {}) => {
    ...
    if (translator.onBatch) {
      translator.onBatch(items.map((item) => item.text), direction);
    }

    const translatedItems = [];
    for (const item of items) {
      const translated = translator.translateBatchItem
        ? await translator.translateBatchItem(item.text, direction)
        : await translator.translate(item.text, direction);
...
// The aligner is now consulted once per sentence even when English hints
// resolve every match, because its opinion is what catches a hint that matched
// by coincidence (see testAlignmentContradictionDropsCoincidentalHint). It must
// still be one check for the whole sentence, not one per match.
async function testEnglishHintsResolveWithOneAlignmentCheck(browser) {
  const page = await browser.newPage();
  ...
  assert(
    alignerCalls === 1,
    `expected exactly one alignment check for the sentence, got ${alignerCalls}`
  );
  await page.close();
}

// "Крім того" means "Additionally". The lemma matcher sees "того" as a form of
// "той" (that), the sentence happens to contain "that" further along, and the
// hint-only pass paired them — overwriting a real "that" with a word that does
// not mean it here. The aligner knows "того" came from "Additionally".
async function testAlignmentContradictionDropsCoincidentalHint(browser) {
  const page = await browser.newPage();
  const source = "Additionally, one can find theologians that take any side.";
  const translated = "Крім того, можна знайти богословів, які беруться за будь-яку сторону.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: createState([
      { id: "e1", source: "that", target: "той", enabled: true, createdAt: 1 },
      { id: "e2", source: "can", target: "можна", enabled: true, createdAt: 2 }
    ]),
    translator: {
      availability: async () => "available",
      translate: async (text) => (text === source ? translated : "")
    ...
    wordAligner: async (sourceText, translatedText) => {
      ...
      // A word-level aligner links "того" to "Additionally", not to "that".
      link("Additionally", "того");
      link("can", "можна");
      return pairs;
    }
  });
  ...
  assert(
    !result.replacements.some((item) => item.text === "того"),
    `a coincidental English hint was still used: ${JSON.stringify(result)}`
  );
  assert(
    result.text.includes("theologians that take"),
    `the unrelated "that" was overwritten: ${JSON.stringify(result)}`
  );
  // The contradicted match is dropped outright rather than moved onto the span
  // the aligner points at, which would teach "того" = "Additionally".
  assert(
    result.text.startsWith("Additionally,"),
    `the contradicted match was painted over its aligned span: ${JSON.stringify(result)}`
  );
  assert(
    result.replacements.some((item) => item.original === "can" && item.text === "можна"),
    `an agreeing hint was dropped too: ${JSON.stringify(result)}`
  );
  await page.close();
}

// Ukrainian puts the possessor behind the noun in the genitive, so reusing the
// English possessive verbatim stranded it: "texts St Augustine's". The gloss
// should say what the case ending does — "texts of-St Augustine".
async function testStructureModeGlossesEnglishPossessiveAsOf(browser) {
  const page = await browser.newPage();
  const source = "We got new St Augustine's texts before GTA VI.";
  const translated = "Ми отримали нові тексти Святого Августина перед GTA VI.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: {
      ...createState([
        { id: "e1", source: "we", target: "ми", enabled: true, createdAt: 1 },
        { id: "e2", source: "new", target: "нові", enabled: true, createdAt: 2 }
      ]),
      structureMode: true
    },
    translator: {
  ...
  const result = await page.evaluate(() => ({
    ...
    ).map((node) => `${node.dataset.learnedWordOriginal}→${node.textContent}`)
  }));

  assert(
    result.text === "Ми got нові texts of-St Augustine before GTA VI.",
    `possessive was not glossed into Ukrainian structure: ${JSON.stringify(result)}`
  );
  // "of-" fronts the whole possessor phrase, and the case marker is gone from
  // its tail rather than being carried along.
  assert(
    result.scaffold.includes("Святого→of-St") && result.scaffold.includes("Августина→Augustine"),
    `the gloss landed on the wrong word: ${JSON.stringify(result.scaffold)}`
  );
  await page.close();
}

// A learned word is left in Ukrainian and the reader takes its English off it,
// so the English it stands for must be spent. It was not, and the next word
// said the pronoun over again: "Я радий" — which is "I glad", Ukrainian having
// no copula — came out as "Я I'm glad", read aloud as "I I'm glad".
async function testLearnedWordSpendsItsEnglishOnce(browser) {
  const page = await browser.newPage();
  const source = "I'm glad that Anna can notice Ukrainian.";
  const translated = "Я радий, що Анна може помітити українську.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "I", target: "я", enabled: true, createdAt: 1 }]),
      structureMode: true
    },
    translator: {
      availability: async () => "available",
    ...
    config: {
      ukrainianLemmas: { я: ["я"], що: ["що"] }
    },
    wordAligner: async (sourceText, translatedText) => {
      const span = (english, ukrainian) => ({
        srcStart: sourceText.indexOf(english),
        srcEnd: sourceText.indexOf(english) + english.length,
        tgtStart: translatedText.indexOf(ukrainian),
        tgtEnd: translatedText.indexOf(ukrainian) + ukrainian.length,
        score: 0.9
      });

      return [
        // The aligner links the pronoun to the learned word AND drags it into
        // the adjective, which is how the duplicate got in.
        span("I'm", "Я"),
        span("I'm", "радий"),
        span("glad", "радий"),
        span("that", "що"),
        span("Anna", "Анна"),
        span("can", "може"),
        span("notice", "помітити"),
        span("Ukrainian", "українську")
      ];
    }
  });
  ...
  assert(
    result.text === "Я glad, that Anna can notice Ukrainian.",
    `the learned word's English was spent twice: ${JSON.stringify(result)}`
  );
  assert(
    result.scaffold.includes("радий→glad"),
    `the pronoun was repeated onto the next word: ${JSON.stringify(result.scaffold)}`
  );
  await page.close();
}

// Scaffold English took its case from the Ukrainian word it replaced, so a
// lowercase Ukrainian token lowercased the English with it. Ukrainian writes
// the pronoun and its language names in lower case, which turned "I'm" into
// "i'm" and "Ukrainian" into "ukrainian".
async function testStructureModeKeepsEnglishOwnCapitals(browser) {
  const page = await browser.newPage();
  const source = "Anna knows I'm learning Ukrainian.";
  const translated = "\u0410\u043d\u043d\u0430 \u0437\u043d\u0430\u0454, \u0449\u043e \u044f \u0432\u0438\u0432\u0447\u0430\u044e \u0443\u043a\u0440\u0430\u0457\u043d\u0441\u044c\u043a\u0443.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: {
      // No entry matches this sentence, so every word is scaffold English and
      // nothing here depends on learned-word handling.
      ...createState([{ id: "e1", source: "dog", target: "\u0441\u043e\u0431\u0430\u043a\u0430", enabled: true, createdAt: 1 }]),
      structureMode: true
    },
    translator: {
      availability: async () => "available",
    ...
    wordAligner: async (sourceText, translatedText) => {
      ...
      link("knows", "\u0437\u043d\u0430\u0454");
      link("I'm", "\u044f");
      link("learning", "\u0432\u0438\u0432\u0447\u0430\u044e");
      link("Ukrainian", "\u0443\u043a\u0440\u0430\u0457\u043d\u0441\u044c\u043a\u0443");
      return pairs;
    }
  });
  ...
  assert(
    result.text === "Anna knows, \u0449\u043e I'm learning Ukrainian.",
    `English lost its own capitals to the Ukrainian word case: ${JSON.stringify(result)}`
  );
  assert(
    result.scaffold.includes("\u044f\u2192I'm"),
    `the pronoun "I" was lowercased: ${JSON.stringify(result.scaffold)}`
  );
  assert(
    result.scaffold.includes("\u0443\u043a\u0440\u0430\u0457\u043d\u0441\u044c\u043a\u0443\u2192Ukrainian"),
    `a language name was lowercased: ${JSON.stringify(result.scaffold)}`
  );
  await page.close();
}
...
// Full translation is structure mode with the English scaffold switched off:
// the whole sentence is painted in the target language and the aligner's
// English only rides along in the span for hovering.
async function testFullTranslationPaintsTheWholeSentenceInTargetLanguage(browser) {
  const page = await browser.newPage();
  const source = "Radio is the technology of communicating.";
  await page.setContent('<p>Radio is the technology of <a href="#">communicating</a>.</p>');
  await installHarness(page, {
    ...
    config: {
      // Every word on screen is Ukrainian now, so every word is vetted against
      // the morphology dictionary — not just the learned one.
      ukrainianLemmas: {
        "радіо": ["радіо"],
  ...
  const result = await page.evaluate(() => {
    const known = document.querySelector(
      '.learned-word-replacer-token:not([data-learned-word-match-kind="unlearned"])'
    );
    const unlearned = [
      ...document.querySelectorAll(
    ...
    return {
      text: document.body.innerText,
      known: { text: known?.textContent, original: known?.dataset.learnedWordOriginal },
      unlearned: unlearned.map((span) => `${span.textContent}→${span.dataset.learnedWordOriginal}`),
      scaffoldCount: document.querySelectorAll(
        '.learned-word-replacer-token[data-learned-word-match-kind="back-translation"]'
      ).length
    };
  });
  ...
  assert(
    result.text === "Радіо — це технологія зв'язку.",
    `full translation did not paint the whole sentence in Ukrainian: ${JSON.stringify(result)}`
  );
  assert(
    result.scaffoldCount === 0,
    `full translation still swapped English into the sentence: ${result.scaffoldCount} scaffold words`
  );
  assert(
    result.known.text === "Радіо" && result.known.original === "Radio",
    `the learned word lost its English original: ${JSON.stringify(result.known)}`
  );
  // The English is not painted, but hovering has to be able to reach it.
  assert(
    result.unlearned.join(",") === "це→is,технологія→technology,зв'язку→communicating",
    `unlearned words did not carry their English for hovering: ${JSON.stringify(result.unlearned)}`
  );
  await page.close();
}

// The aligner only supplies hover English here, so a page must still come out
// fully translated when it has nothing to say — structure mode keeps the
// English sentence in that case, and inheriting that would have left whole
// paragraphs untranslated whenever the alignment model was unavailable.
async function testFullTranslationPaintsWithoutTheAligner(browser) {
  const page = await browser.newPage();
  const source = "Radio is the technology of communicating.";
  await page.setContent("<p>Radio is the technology of communicating.</p>");
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "radio", target: "радіо", enabled: true, createdAt: 1 }]),
      fullTranslation: true
  ...
  assert(
    text === "Радіо — це технологія зв'язку.",
    `an unaligned block was not translated: ${text}`
  );
  await page.close();
}

// The whole point of the setting: nothing goes back into English, including on
// a page that arrived in the target language with "Read target-language pages"
// still switched on.
async function testFullTranslationLeavesTargetLanguagePagesAlone(browser) {
  const page = await browser.newPage();
  const fixture = createTargetLanguagePageFixture({ fullTranslation: true });
  await page.setContent(fixture.html);
  const before = await page.evaluate(() => document.getElementById("uk-copy").innerHTML);
  await installHarness(page, fixture.harness);

  await page.waitForFunction(
    () => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0
  );
  const result = await page.evaluate(() => ({
    html: document.getElementById("uk-copy").innerHTML,
  ...
  assert(
    result.html === before,
    `English was swapped into a target-language page: ${result.html}`
  );
  assert(
    result.tokens === 0,
    `the reverse pass still ran under full translation: ${result.tokens} tokens`
  );
  assert(
    result.unitsCollected === 0,
    `target-language blocks were still collected: ${result.unitsCollected} units`
  );
  await page.close();
}

// Full translation paints target-language text into blocks that had nothing
// learned in them, which is exactly the text the collector treats as a
// target-language page. If those repainted blocks are collected again the page
// never settles, so prove a finished page stops feeding itself work.
async function testFullTranslationSettlesInsteadOfReprocessingItself(browser) {
  const page = await browser.newPage();
  const sentences = {
    "Radio is the technology of communicating.": "Радіо - це технологія зв'язку.",
    "The house is very old.": "Будинок дуже старий.",
    "We read the book together.": "Ми читаємо книгу разом."
  };
  await page.setContent(
  ...
  await installHarness(page, {
    ...
    translator: {
      availability: async () => "available",
      translate: async (text) => sentences[text] || ""
    },
    config: {
      ukrainianLemmas: {
        "радіо": ["радіо"],
  ...
  const settled = await page.evaluate(async () => {
    ...
    const baseRunId = snapshot().runId;
    // The last pass's stats stay on the snapshot, so watch for passes that
    // start after this point and record what each of them picks up.
    const laterPasses = [];
    const watch = setInterval(() => {
      const status = snapshot();
      if (status.runId > baseRunId) {
        laterPasses.push(status.unitsCollected);
      }
    }, 15);

    // Poke the page the way scrolling and dynamic content do.
  ...
  assert(
    settled.text ===
      "Радіо — це технологія зв'язку.\n\nБудинок дуже старий.\n\nМи читаємо книгу разом.",
    `full translation did not paint every block: ${JSON.stringify(settled.text)}`
  );
  assert(
    settled.laterPasses.every((units) => units === 0),
    `a settled page kept collecting its own translated blocks: ${JSON.stringify(settled.laterPasses)}`
  );
  await page.close();
}

// Repainting a block rebuilds its children, which used to leave a nav item's
// <a> behind as bare text: the link stopped working and the site's own `li a`
// styling stopped applying, so the items ran together on screen. The aligner
// already says which English each translated word came from, so the element
// around that English is rebuilt around the word that replaced it.
async function testFullTranslationKeepsLinksAroundTheirTranslatedWords(browser) {
  const page = await browser.newPage();
  const source = "Radio is the technology of communicating.";
  await page.setContent(
    '<p>Radio is the technology of <a href="/comms" class="nav">communicating</a>.</p>'
  );
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "radio", target: "радіо", enabled: true, createdAt: 1 }]),
  ...
  const result = await page.evaluate(() => {
    const link = document.querySelector("a[href='/comms']");
    return {
      text: document.body.innerText,
      linkText: link?.textContent,
      linkClass: link?.className,
      linkHoldsToken: Boolean(link?.querySelector(".learned-word-replacer-token"))
    };
  });

  assert(
    result.text === "Радіо — це технологія зв'язку.",
    `the sentence was not fully translated: ${JSON.stringify(result.text)}`
  );
  assert(
    result.linkText === "зв'язку",
    `the link did not survive with its translated word: ${JSON.stringify(result)}`
  );
  assert(
    result.linkClass === "nav" && result.linkHoldsToken,
    `the rebuilt link lost its attributes or its word: ${JSON.stringify(result)}`
  );
  await page.close();
}

// Found live on YouTube: its sidebar scrolls inside its own pane, and scroll
// events do not bubble, so a listener on the window never heard it — the
// entries scrolled into view stayed English. The capture phase hears them.
async function testTextScrolledIntoViewInsideAPaneIsTranslated(browser) {
  const page = await browser.newPage({ viewport: { width: 800, height: 400 } });
  await page.setContent(`
    <div id="pane" style="height:300px;overflow-y:scroll">
      <p id="first">The house is very old.</p>
      <p id="far" style="display:none">The water is very cold.</p>
    </div>
  `);
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "house", target: "\u0431\u0443\u0434\u0438\u043d\u043e\u043a", enabled: true, createdAt: 1 }]),
      fullTranslation: true
    },
    translator: {
      availability: async () => "available",
      translate: async (text) => {
        if (text === "The house is very old.") {
          return "\u0411\u0443\u0434\u0438\u043d\u043e\u043a \u0434\u0443\u0436\u0435 \u0441\u0442\u0430\u0440\u0438\u0439.";
        }
        if (text === "The water is very cold.") {
          return "\u0412\u043e\u0434\u0430 \u0434\u0443\u0436\u0435 \u0445\u043e\u043b\u043e\u0434\u043d\u0430.";
        }
        return "";
      }
    },
  ...
  await page.waitForFunction(() =>
    document.getElementById("first").innerText.includes("\u0411\u0443\u0434\u0438\u043d\u043e\u043a")
  );

  // Reveal it the way an expander does: an attribute change, which the observer
  // does not watch, so nothing is queued by this alone.
  await page.evaluate(() => {
    document.getElementById("far").style.display = "";
  });
  await page.waitForTimeout(400);
  assert(
    !(await page.evaluate(() =>
      document.getElementById("far").innerText.includes("\u0412\u043e\u0434\u0430")
    )),
    "revealing text on its own should not have translated it yet"
  );

  // Scrolling the pane fires a scroll event on the pane, which never reaches a
  // listener on the window.
  await page.evaluate(() => {
    document.getElementById("pane").dispatchEvent(new Event("scroll"));
  });
  await page.waitForFunction(
    () => document.getElementById("far").innerText.includes("\u0412\u043e\u0434\u0430"),
    { timeout: 10000 }
...
// Found live on YouTube: a good translation was thrown away because the
// morphology dictionary does not list every compound Ukrainian forms — one
// unlisted word left the whole paragraph in English. Full translation asks
// whether most of the sentence is unknown, which is what a made-up sentence
// looks like; a single compound is not.
async function testFullTranslationKeepsSentencesWithOneUnlistedWord(browser) {
  const page = await browser.newPage();
  const source = "A friend of mine took my video catalogue and made a torrent.";
  const translated = "Мій друг взяв мій відеокаталог і зробив торрент.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "friend", target: "друг", enabled: true, createdAt: 1 }]),
      fullTranslation: true
    },
    translator: {
      availability: async () => "available",
    ...
    config: {
      // "відеокаталог" is missing, exactly as it is from the real dictionary.
      ukrainianLemmas: {
        "мій": ["мій"],
        "друг": ["друг"],
        "взяв": ["взяти"],
  ...
  assert(
    text === translated,
    `one unlisted compound threw away the whole sentence: ${JSON.stringify(text)}`
  );
  await page.close();
}

// Found live on YouTube: the subscription list is in the DOM all along and a
// "Show more" expander only unhides it. That is an attribute change, which the
// observer does not watch, so 55 revealed entries stayed English until
// something else happened to trigger a pass.
async function testRevealedTextIsTranslatedAfterAClick(browser) {
  const page = await browser.newPage();
  await page.setContent(`
    <p id="visible">The house is very old.</p>
    <div id="more" style="display:none"><p id="hidden">The water is very cold.</p></div>
    <span id="expander">Show more</span>
  `);
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "house", target: "будинок", enabled: true, createdAt: 1 }]),
  ...
  await page.waitForFunction(() =>
    document.getElementById("visible").innerText.includes("Будинок")
  );
  // Unhide it the way an expander does — no nodes added, no text changed.
  await page.evaluate(() => {
    document.getElementById("more").style.display = "";
  });
  await page.waitForTimeout(400);
  assert(
    !(await page.evaluate(() => document.getElementById("hidden").innerText.includes("Вода"))),
    "test setup should leave the revealed block untranslated until the click"
  );

  await page.click("#expander");
  await page.waitForFunction(
    () => document.getElementById("hidden").innerText.includes("Вода"),
    { timeout: 10000 }
  );
  await page.close();
}

// Found live on YouTube: "Live with Rabbi Yosef Mizrachi" translated correctly
// to "Живіть з рабином Йосефом Мізрачі" and was thrown away, because a
// transliterated name is never in the morphology dictionary and full
// translation vets every word on screen. A word standing in for a capitalised
// English word is taken on trust; ordinary lowercase words stay vetted.
async function testFullTranslationTrustsTransliteratedNames(browser) {
  const page = await browser.newPage();
  const source = "Live with Rabbi Yosef Mizrachi";
  const translated = "Живіть з рабином Йосефом Мізрачі";
  await page.setContent(`<h3>${source}</h3>`);
  await installHarness(page, {
    state: {
    ...
    config: {
      // The dictionary knows none of the transliterated names, exactly as the
      // real one does not.
      ukrainianLemmas: { "живіть": ["жити"] }
    },
    wordAligner: async (sourceText, translatedText) => {
      const pairs = [];
      const link = (english, ukrainian) => {
  ...
  assert(
    text === translated,
    `a title full of names was rejected instead of painted: ${JSON.stringify(text)}`
  );
  await page.close();
}

// Found live on YouTube: every sidebar entry is a link holding an avatar and a
// label, and banning blocks that contain an <svg> or <img> left the whole
// sidebar in English. The picture holds no words, so it is carried across
// untouched instead of disqualifying the block.
async function testFullTranslationCarriesIconsThroughTheRebuild(browser) {
  const page = await browser.newPage();
  const source = "The house is very old.";
  await page.setContent(
    '<h3><a id="endpoint" href="/c"><svg id="avatar" width="10" height="10"><circle r="4"/></svg>' +
      '<span class="label">The house is very old.</span>' +
      '<img id="badge" alt="" width="8" height="8"></a></h3>'
  );
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "house", target: "будинок", enabled: true, createdAt: 1 }]),
    ...
    translator: {
      availability: async () => "available",
      translate: async (text) => (text.trim() === source ? "Будинок дуже старий." : "")
    },
    config: {
      ukrainianLemmas: { "будинок": ["будинок"], "дуже": ["дуже"], "старий": ["старий"] }
    },
    wordAligner: async (sourceText, translatedText) => {
      const pairs = [];
      const link = (english, ukrainian) => {
  ...
  const result = await page.evaluate(() => {
    const link = document.querySelector("a#endpoint");
    return {
      text: document.body.innerText.trim(),
      avatar: Boolean(document.querySelector("svg#avatar circle")),
      badge: Boolean(document.querySelector("img#badge")),
      // The avatar sat before the words and the badge after them.
      order: [...(link?.children || [])].map((child) => child.id || child.tagName).join(","),
      label: document.querySelector("span.label")?.textContent
    };
  });

  assert(
    result.text === "Будинок дуже старий.",
    `a block holding an icon was not translated: ${JSON.stringify(result.text)}`
  );
  assert(result.avatar, `the avatar was dropped: ${JSON.stringify(result)}`);
  assert(result.badge, `the trailing image was dropped: ${JSON.stringify(result)}`);
  assert(
    result.order === "avatar,SPAN,badge",
    `the icons did not keep their side of the words: ${result.order}`
  );
  assert(
    result.label === "Будинок дуже старий.",
    `the words did not land inside their own wrapper: ${JSON.stringify(result.label)}`
  );
  await page.close();
}

// Found live on YouTube: pages keep their text in custom elements
// (<yt-formatted-string>, <tp-yt-paper-item>), and an allow-list of HTML inline
// tags treated every one of them as unsafe markup — so full translation quietly
// fell back to per-word replacement on the whole sidebar and left it English.
// The wrappers are rebuilt around their own words, nesting and all.
async function testFullTranslationRebuildsCustomElementWrappers(browser) {
  const page = await browser.newPage();
  const source = "The house is very old.";
  await page.setContent(
    '<h3><a id="endpoint" href="/watch"><yt-formatted-string class="title">' +
      "The house is very old." +
      "</yt-formatted-string></a></h3>"
  );
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "house", target: "будинок", enabled: true, createdAt: 1 }]),
    ...
    translator: {
      availability: async () => "available",
      translate: async (text) => (text === source ? "Будинок дуже старий." : "")
    },
    config: {
      ukrainianLemmas: {
        "будинок": ["будинок"],
  ...
  // Mark the page's own element so the test can tell it apart from a copy.
  await page.evaluate(() => {
    document.querySelector("yt-formatted-string").__lwrIdentity = "original";
  });
  await page.waitForFunction(
    () => document.querySelector(".learned-word-replacer-structured") !== null
  );
  const result = await page.evaluate(() => {
    const custom = document.querySelector("yt-formatted-string");
    return {
      text: document.body.innerText.trim(),
      sameCustomNode: custom?.__lwrIdentity === "original",
      hasCustom: Boolean(custom),
      customClass: custom?.className,
      nestedInLink: custom?.parentElement?.tagName,
  ...
  assert(
    result.text === "Будинок дуже старий.",
    `the custom-element block was not fully translated: ${JSON.stringify(result.text)}`
  );
  assert(
    result.hasCustom && result.customClass === "title",
    `the custom element was flattened away: ${JSON.stringify(result)}`
  );
  // It has to be the page's own element, not a copy: copying a framework
  // element makes an instance that re-renders itself empty.
  assert(
    result.sameCustomNode,
    `the custom element was replaced by a copy of itself: ${JSON.stringify(result)}`
  );
  // The chain is rebuilt in order, not collapsed: the custom element still sits
  // inside the link it came from.
  assert(
    result.nestedInLink === "A" && result.linkHref === "/watch",
    `the element chain was not rebuilt in order: ${JSON.stringify(result)}`
  );
  assert(
    result.customText === "Будинок дуже старий.",
    `the words did not land inside their own wrapper: ${JSON.stringify(result.customText)}`
  );
  await page.close();
}

// Found live on YouTube: a video title is one block-level <a>, and rebuilding
// it as one clone per run of parts cut it apart at every comma. Each piece
// became its own block-level link, so the punctuation between them landed on
// lines of its own. One element gets one copy, first word to last, with the
// punctuation inside it.
async function testFullTranslationKeepsPunctuationInsideOneLinkCopy(browser) {
  const page = await browser.newPage();
  const source = "The house is old, and the water is cold.";
  const translated = "Будинок старий, а вода холодна.";
  // The YouTube shape: a heading is the block, and the whole title inside it is
  // one block-level link.
  await page.setContent(`<h3><a id="title" href="/watch" style="display:block">${source}</a></h3>`);
  await installHarness(page, {
    state: {
      ...createState([{ id: "e1", source: "house", target: "будинок", enabled: true, createdAt: 1 }]),
      fullTranslation: true
  ...
  const result = await page.evaluate(() => ({
    text: document.body.innerText,
    links: document.querySelectorAll("a[href='/watch']").length,
    linkText: document.querySelector("a[href='/watch']")?.textContent,
    strayText: [...document.querySelector("h3").childNodes]
      .filter((node) => node.nodeType === Node.TEXT_NODE)
      .map((node) => node.nodeValue)
      .join("")
  }));

  assert(result.links === 1, `the link was cut into ${result.links} copies`);
  assert(
    result.linkText === translated,
    `the link lost part of its sentence: ${JSON.stringify(result.linkText)}`
  );
  // The comma and full stop have to sit inside the link, not next to it, or a
  // block-level link puts them on their own line.
  assert(
    !result.strayText.trim(),
    `punctuation was left outside the link: ${JSON.stringify(result.strayText)}`
  );
  assert(
    result.text.replace(/\s+/gu, " ").trim() === translated,
    `the painted title did not read as one line: ${JSON.stringify(result.text)}`
  );
  await page.close();
}

// Found live on a long page: a pass takes the highest-priority 35 blocks and
// drops the rest, so the page translated down to a point and simply stopped —
// the leftovers waited for a scroll that may never come. A pass that leaves
// work behind has to pick it up itself.
async function testTranslationContinuesPastOnePassBudget(browser) {
  const page = await browser.newPage();
  const sentences = {
    "The house is very old.": "Будинок дуже старий.",
    "The book is very old.": "Книга дуже стара.",
    "The city is very old.": "Місто дуже старе.",
    "The sea is very cold.": "Море дуже холодне.",
    "The water is very cold.": "Вода дуже холодна.",
    "The bread is very old.": "Хліб дуже старий."
  };
  await page.setContent(
    Object.keys(sentences)
      .map((text) => `<p>${text}</p>`)
  ...
  await installHarness(page, {
    ...
    config: {
      // Two blocks per pass against six blocks: without the continuation the
      // page stops after the first two.
  ...
  // Nothing scrolls and nothing else touches the page: if the passes do not
  // chain, this never reaches six and the wait fails.
  await page.waitForFunction(
    () => document.querySelectorAll(".learned-word-replacer-structured").length === 6,
    { timeout: 15000 }
  );
  const result = await page.evaluate(() => ({
    text: document.body.innerText.replace(/\s+/gu, " ").trim(),
    passes: window.__learnedWordReplacerDebug.getSnapshot().runId
  }));

  assert(
    result.text ===
      "Будинок дуже старий. Книга дуже стара. Місто дуже старе. Море дуже холодне. Вода дуже холодна. Хліб дуже старий.",
    `the page did not finish translating: ${result.text}`
  );
  assert(result.passes >= 3, `six blocks at two per pass took ${result.passes} passes`);
  await page.close();
}
...
// Ukrainian writes the hard separator as an apostrophe, so "православ'я" is one
// word. Treating the apostrophe as a boundary let the learned entry "я" match
// the tail of that word, and the aligner then painted it over "Orthodoxy's".
async function testUkrainianApostropheIsNotAWordBoundary(browser) {
  const page = await browser.newPage();
  const source = "Orthodoxy's error on the Eucharist.";
  const translated = "Помилка православ'я на Євхаристії.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: createState([
      { id: "e1", source: "I", target: "я", enabled: true, createdAt: 1 },
      { id: "e2", source: "on", target: "на", enabled: true, createdAt: 2 }
    ]),
    translator: {
      availability: async () => "available",
      translate: async (text) => (text === source ? translated : "")
    ...
    wordAligner: async (sourceText, translatedText) => {
      const tailStart = translatedText.indexOf("православ'я") + "православ'".length;
      const pairs = [];

      if (tailStart >= "православ'".length && sourceText.includes("Orthodoxy's")) {
        pairs.push({
          srcStart: sourceText.indexOf("Orthodoxy's"),
          srcEnd: sourceText.indexOf("Orthodoxy's") + "Orthodoxy's".length,
          tgtStart: tailStart,
          tgtEnd: tailStart + 1,
          score: 0.9
        });
      ...
      if (sourceText.includes(" on ") && translatedText.includes(" на ")) {
        pairs.push({
          srcStart: sourceText.indexOf(" on ") + 1,
          srcEnd: sourceText.indexOf(" on ") + 3,
          tgtStart: translatedText.indexOf(" на ") + 1,
          tgtEnd: translatedText.indexOf(" на ") + 3,
          score: 0.9
        });
      }
  ...
  assert(
    !result.replacements.some((item) => item.text === "я"),
    `letter after a Ukrainian apostrophe was matched as a standalone word: ${JSON.stringify(result)}`
  );
  assert(result.text.includes("Orthodoxy's"), "possessive was swallowed by an apostrophe fragment");
  assert(
    result.replacements.some((item) => item.original === "on" && item.text === "на"),
    `genuine standalone word was not replaced: ${JSON.stringify(result)}`
  );
  await page.close();
}

// Ukrainian glues compounds with a hyphen, so "будь-яку" is one word. Splitting
// it handed the word-family matcher two fragments that collide with very common
// entries — "будь" (imperative of "бути") and "яку" (a form of "який").
async function testUkrainianHyphenCompoundIsNotSplit(browser) {
  const page = await browser.newPage();
  const source = "Theologians take every side of any issue, as you demonstrated.";
  const translated =
    "Богослови беруться за будь-яку сторону будь-якого питання, як ви продемонстрували.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: createState([
      { id: "e1", source: "be", target: "будь", enabled: true, createdAt: 1 },
      { id: "e2", source: "which", target: "яку", enabled: true, createdAt: 2 },
      { id: "e3", source: "you", target: "ви", enabled: true, createdAt: 3 }
    ]),
    translator: {
      availability: async () => "available",
      translate: async (text) => (text === source ? translated : "")
    ...
    config: {
      ukrainianLemmas: {
        ...
        ви: ["ви"],
        "будь-яку": ["будь-який"],
        "будь-якого": ["будь-який"]
      }
    },
    wordAligner: async (sourceText, translatedText) => {
      const pairs = [];
      const pair = (src, tgt) => {
        const srcStart = sourceText.indexOf(src);
        const tgtStart = translatedText.indexOf(tgt);
        if (srcStart >= 0 && tgtStart >= 0) {
          pairs.push({
            srcStart,
            srcEnd: srcStart + src.length,
            tgtStart,
            tgtEnd: tgtStart + tgt.length,
            score: 0.9
          });
        }
      };
  ...
  assert(
    !result.replacements.some((item) => item.text === "будь" || item.text === "яку"),
    `hyphenated compound was split into standalone fragments: ${JSON.stringify(result)}`
  );
  assert(
    result.text.includes("every side of any issue"),
    `compound fragments were painted over unrelated English: ${JSON.stringify(result)}`
  );
  assert(
    result.replacements.some((item) => item.original === "you" && item.text === "ви"),
    `genuine standalone word was not replaced: ${JSON.stringify(result)}`
  );
  await page.close();
}
...
async function testProcessedBlocksAreMarkedOnPage(browser) {
  ...
  // Only the block that ended up unchanged is marked: the other one already
  // shows its replacement underline, so a section marker there is noise.
  await page.waitForFunction(
    () => document.querySelectorAll(".learned-word-replacer-checked").length === 1
  );
  await page.waitForFunction(
    () => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0
  );
  const result = await page.evaluate(() => {
    const marked = [...document.querySelectorAll(".learned-word-replacer-checked")];
    return {
      markedCount: marked.length,
      markedText: marked[0]?.textContent,
      markedHasReplacement: marked.some((block) =>
        block.querySelector(".learned-word-replacer-token")
      ),
      marker: {
        background: getComputedStyle(marked[0], "::before").backgroundColor,
        width: getComputedStyle(marked[0], "::before").width,
        radius: getComputedStyle(marked[0], "::before").borderTopLeftRadius,
        float: getComputedStyle(marked[0], "::before").cssFloat
      },
      styleIncludesMarker: document
        .getElementById("learned-word-replacer-style")
        ?.textContent.includes(`.learned-word-replacer-checked::before`)
    };
  });

  assert(result.markedCount === 1, `expected only the unchanged block to be marked: ${result.markedCount}`);
  assert(
    result.markedText === "Nothing matches here." && !result.markedHasReplacement,
    `the marked block was not the unchanged one: ${JSON.stringify(result)}`
  );
  // A small round dot in the left margin, taking the block's own text colour
  // and floated out of the flow so no text shifts.
  assert(
    result.marker.radius === "50%" &&
      parseFloat(result.marker.width) > 0 &&
      parseFloat(result.marker.width) < 8 &&
      result.marker.float === "left",
    `the checked marker is not a small hanging dot: ${JSON.stringify(result.marker)}`
  );
...
// A rebuilt block whose words are all English scaffold carries no underline at
// all, so without the fox there is no way to tell it apart from untouched page
// text — Wikipedia's "Union of Uzhhorod" heading became "Union Uzhhorod" with
// nothing to show for it.
async function testBlocksRebuiltWithoutVisibleMarksAreStillMarked(browser) {
  const page = await browser.newPage();
  const source = "Radio is the technology of communicating.";
  await page.setContent(`<p>${source}</p>`);
  await installHarness(page, {
    state: {
      // The learned word is deliberately absent from the sentence, so the
      // rebuild produces back-translation scaffold and nothing else.
    ...
    config: { ukrainianLemmas: { "кіт": ["кіт"] } },
    wordAligner: createStructureModeAligner()
  });
  ...
  const result = await page.evaluate(() => {
    ...
    const kinds = [...block.querySelectorAll(".learned-word-replacer-token")].map(
      (span) => span.dataset.learnedWordMatchKind
    );
    return {
      kinds,
      marked: block.classList.contains("learned-word-replacer-checked"),
      underline: getComputedStyle(block.querySelector(".learned-word-replacer-token"))
        .backgroundImage
    };
  });

  assert(
    result.kinds.length > 0 && result.kinds.every((kind) => kind === "back-translation"),
    `expected an all-scaffold rebuild: ${JSON.stringify(result.kinds)}`
  );
  assert(
    result.underline === "none",
    `scaffold words are supposed to carry no underline: ${result.underline}`
  );
  assert(
    result.marked,
    "a block rebuilt with no visible marks was left with no indicator at all"
  );
  await page.close();
}

// The real cloak reads the stored state through the content-script chrome API,
// which the harness only installs later; give it the same shape up front.
async function installCloakStorageStub(page, state = { enabled: true }) {
  await page.evaluate((storedState) => {
    window.chrome = {
      storage: {
        local: {
          get(defaults, callback) {
            callback({ learnedWordReplacerState: storedState });
          }
        }
      }
    };
...
// Hiding text is only free when the extension is about to rewrite it, so a
// page it will never touch gets its text straight back.
async function testCloakLiftsImmediatelyOnExcludedPages(browser) {
  const page = await browser.newPage();
  await page.setContent('<p id="copy">It is in the house and car.</p>');
  // page.setContent serves about:blank, so exclude that exact page.
  await installCloakStorageStub(page, {
    enabled: true,
    doNotTranslate: { sites: [], pages: ["about:blank"] }
  });
  await page.addScriptTag({ path: CLOAK_SCRIPT });

  const cloaked = await page.evaluate(() =>
    Boolean(document.getElementById("learned-word-replacer-cloak-style"))
  );
  assert(!cloaked, "an excluded page was left with its text hidden");
  await page.close();

  const disabledPage = await browser.newPage();
  await disabledPage.setContent('<p id="copy">It is in the house and car.</p>');
  await installCloakStorageStub(disabledPage, { enabled: false });
  await disabledPage.addScriptTag({ path: CLOAK_SCRIPT });
  assert(
    !(await disabledPage.evaluate(() =>
      Boolean(document.getElementById("learned-word-replacer-cloak-style"))
    )),
    "a page was left hidden while the extension was switched off"
  );
  await disabledPage.close();
}

// Found live on Wikipedia: MediaWiki assigns document.documentElement.className
// wholesale during startup, which wiped a marker class the cloak had added and
// let the untranslated text paint anyway. The cloak must not be defeatable by
// anything the page does to <html>.
async function testCloakSurvivesThePageRewritingTheRootClass(browser) {
  const page = await browser.newPage();
  await page.setContent('<p id="copy">It is in the house and car.</p>');
  await installCloakStorageStub(page);
  await page.addScriptTag({ path: CLOAK_SCRIPT });
  ...
  const result = await page.evaluate(() => {
    document.documentElement.className = "client-js vector-feature-zebra";
    document.documentElement.setAttribute("class", "client-js");
    return {
      color: getComputedStyle(document.getElementById("copy")).color,
      stillCloaked: Boolean(document.getElementById("learned-word-replacer-cloak-style"))
    };
  });

  assert(
    result.stillCloaked,
    "the page rewriting <html> class removed the cloak"
  );
  assert(
    result.color === "rgba(0, 0, 0, 0)",
    `text painted after the page rewrote the root class: ${result.color}`
  );
  await page.close();
}

// The whole point of the cloak: at no instant is the untranslated wording
// readable. The test records the text at the exact moment the cloak lifts.
async function testCloakHandsOffToThePendingHide(browser) {
  const page = await browser.newPage();
  const source = "It is in the house and car.";
  let resolveTranslation;
  const translationPromise = new Promise((resolve) => {
    resolveTranslation = resolve;
  });

  await page.setContent(`<p id="copy">${source}</p>`);
  await installCloakStorageStub(page);
  await page.addScriptTag({ path: CLOAK_SCRIPT });
  const cloaked = await page.evaluate(() => ({
    cloaked: Boolean(document.getElementById("learned-word-replacer-cloak-style")),
    color: getComputedStyle(document.getElementById("copy")).color
  }));
  assert(cloaked.cloaked, "the cloak did not hide the page at document_start");
  assert(
    cloaked.color === "rgba(0, 0, 0, 0)",
    `page text was still painted while cloaked: ${cloaked.color}`
  );

  // Record the block's state at the exact instant the cloak stylesheet is
  // pulled: the per-block hide must already have taken over.
  await page.evaluate(() => {
    window.__atLift = null;
    new MutationObserver(() => {
      if (window.__atLift === null && !document.getElementById("learned-word-replacer-cloak-style")) {
        const copy = document.getElementById("copy");
        window.__atLift = {
          pending: copy.hasAttribute("data-lwr-pending"),
  ...
  await installHarness(page, {
    ...
    translator: {
      availability: async () => "available",
      translate: async (text) => (text === source ? await translationPromise : "")
    }
  });

  await page.waitForFunction(() => window.__atLift !== null, null, { timeout: 3000 });
  const lift = await page.evaluate(() => ({
    atLift: window.__atLift,
    styleGone: !document.getElementById("learned-word-replacer-cloak-style")
  }));

  // The handoff: the page-wide cloak only lets go once this block is carrying
  // its own hide, so the untranslated wording is never on screen for a frame.
  assert(
    lift.atLift.pending,
    "the page was handed back before the block was hidden in its own right"
  );
  assert(
    lift.atLift.color === "rgba(0, 0, 0, 0)",
    `the original wording was readable the moment the page was handed back: ${lift.atLift.color}`
  );
  assert(
    lift.atLift.text === source,
    `the page's own words were altered: ${JSON.stringify(lift.atLift.text)}`
  );
  assert(lift.styleGone, "the cloak stylesheet was left behind");

  resolveTranslation("Це у будинку і авто.");
  await page.waitForFunction(() => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0);
  ...
  assert(
    settled.text === "It is in the будинку and car." && settled.color !== "rgba(0, 0, 0, 0)",
    `the block did not appear translated after the cloak lifted: ${JSON.stringify(settled)}`
  );
  await page.close();
}

// content.js may never arrive — an extension reload re-injects it without the
// cloak, a storage read can hang. The cloak has to let go on its own.
async function testCloakLiftsItselfWithoutTheContentScript(browser) {
  const page = await browser.newPage();
  await page.setContent('<p id="copy">It is in the house and car.</p>');
  await installCloakStorageStub(page);
  await page.addScriptTag({ path: CLOAK_SCRIPT });
  assert(
    ...
    "the cloak did not apply"
  );

  await page.waitForFunction(
    () => !document.getElementById("learned-word-replacer-cloak-style"),
    null,
    { timeout: 8000 }
  );
  ...
  assert(color !== "rgba(0, 0, 0, 0)", `the page was left unreadable: ${color}`);
  await page.close();
}

function testCloakRunsBeforeThePagePaints() {
...
async function testPendingBlocksStayHiddenUntilTheirTranslationLands(browser) {
  ...
  await page.setContent(`<p id="copy">${source}</p>`);
  // Hiding only happens on a page that has not been shown yet, so these
  // loads start the way a real one does: cloaked at document_start.
  await installCloakStorageStub(page);
  await page.addScriptTag({ path: CLOAK_SCRIPT });
  await installHarness(page, {
    state: {
  ...
  await page.waitForFunction(
    () => document.getElementById("copy").hasAttribute("data-lwr-pending"),
    null,
    { timeout: 3000 }
  );
  const waiting = await page.evaluate(() => ({
    color: getComputedStyle(document.getElementById("copy")).color,
    // The words themselves are untouched — only their colour is — so nothing
    // can corrupt the page text and nothing reflows when it appears.
    text: document.getElementById("copy").textContent,
    width: Math.round(document.getElementById("copy").getBoundingClientRect().width)
  }));

  assert(
    waiting.color === "rgba(0, 0, 0, 0)",
    `the untranslated wording was readable: ${waiting.color}`
  );
  assert(
    waiting.text === source,
    `the page's own text was altered while it waited: ${waiting.text}`
  );
  ...
  const settled = await page.evaluate(() => ({
    text: document.getElementById("copy").textContent,
    color: getComputedStyle(document.getElementById("copy")).color,
    pending: document.getElementById("copy").hasAttribute("data-lwr-pending"),
    tokens: document.querySelectorAll(".learned-word-replacer-token").length,
    width: Math.round(document.getElementById("copy").getBoundingClientRect().width)
  }));

  assert(!settled.pending, "the block was left waiting after its translation landed");
  assert(
    settled.color !== "rgba(0, 0, 0, 0)",
    `the block stayed invisible after it was done: ${settled.color}`
  );
  assert(
    settled.text === "It is in the \u0431\u0443\u0434\u0438\u043d\u043a\u0443 and car.",
    `the block did not settle into its translated text: ${settled.text}`
  );
  assert(settled.tokens === 1, "the learned word was not replaced");
  assert(
    settled.width === waiting.width,
    `the block changed width when it appeared: ${waiting.width} -> ${settled.width}`
  );
  await page.close();
}

// Text vanishing from under someone who is already reading is worse than
// watching it swap, so nothing is ever blanked once the page has been handed
// over — only the first paint hides anything.
async function testScrollingIntoNewTextNeverBlanksIt(browser) {
  const page = await browser.newPage();
  ...
  await installHarness(page, {
    ...
    translator: {
      availability: async () => "available",
      translate: async (text) =>
        text === "It is in the house." ? "Це у будинку." : "Це моє авто."
    }
  });

  await page.waitForFunction(() => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0);
  assert(
    await page.evaluate(() => !document.getElementById("first").hasAttribute("data-lwr-pending")),
    "the first block never came back"
  );

  // New text arrives the way scrolling brings it in.
  await page.evaluate(() => {
    window.__blanked = [];
    new MutationObserver((records) => {
      for (const record of records) {
        if (record.target.hasAttribute("data-lwr-pending")) {
          window.__blanked.push(record.target.id || record.target.tagName);
        }
      }
    }).observe(document.body, {
      attributes: true,
      attributeFilter: ["data-lwr-pending"],
      subtree: true
    });

    const added = document.createElement("p");
    added.id = "later";
    added.textContent = "This is my car.";
    document.body.append(added);
  });

  await page.waitForFunction(
    () => document.getElementById("later").textContent.includes("авто"),
    null,
    { timeout: 3000 }
  );
  const result = await page.evaluate(() => ({
    blanked: window.__blanked,
    laterText: document.getElementById("later").textContent,
    laterColor: getComputedStyle(document.getElementById("later")).color
  }));

  assert(
    result.blanked.length === 0,
    `text was blanked out after the page was handed over: ${JSON.stringify(result.blanked)}`
  );
  assert(
    result.laterText === "This is my авто." && result.laterColor !== "rgba(0, 0, 0, 0)",
    `the new text was not translated in place: ${JSON.stringify(result)}`
  );
  await page.close();
}

// The one thing that must never happen is leaving a page unreadable.
async function testHiddenTextIsRevealedWhenTranslationFails(browser) {
  const page = await browser.newPage();
  const source = "It is in the house and car.";
  await page.setContent(`<p id="copy">${source}</p>`);
  // Hiding only happens on a page that has not been shown yet, so these
  ...
  assert(!result.pending, "a failed translation left the block hidden");
  assert(
    result.color !== "rgba(0, 0, 0, 0)" && result.text === source,
    `a failed translation left the page unreadable: ${JSON.stringify(result)}`
  );
  await page.close();

  const offPage = await browser.newPage();
  await offPage.setContent(`<p id="copy">${source}</p>`);
  await installHarness(offPage, {
    state: createState([
      { id: "e1", source: "house", target: "\u0431\u0443\u0434\u0438\u043d\u043a\u0443", enabled: true, createdAt: 1 }
    ]),
    translator: {
      availability: async () => "available",
      translate: async () => new Promise(() => {})
    }
  });
  await offPage.waitForFunction(
    () => window.__learnedWordReplacerDebug.getSnapshot().status === "translating"
  );
  const offResult = await offPage.evaluate(() =>
    getComputedStyle(document.getElementById("copy")).color
  );
  assert(
    offResult !== "rgba(0, 0, 0, 0)",
    `text was hidden while the setting was off: ${offResult}`
  );
  await offPage.close();
}

// Found live on a long page: blocks that yield no translation, and blocks that
// wrap nested blocks, were never recorded as processed, so every scroll pass
// collected them again and churned text the reader had already read.
async function testAlreadyReadBlocksNeverChurnAgain(browser) {
  const page = await browser.newPage();
  // The outer div wraps a nested block, which is what made the stored unit
  // text disagree with the block's own source text.
  await page.setContent(`
    <div id="outer">Wrapper text here.<p id="inner">It is in the house.</p></div>
    <p id="untranslatable">3:30 pm</p>
  `);
  ...
  await installHarness(page, {
    ...
    translator: {
      availability: async () => "available",
      // Anything but the one real sentence comes back empty, like the short
      // labels and times that never translate on a real page.
      translate: async (text) => {
        translateCalls.push(text);
        return text === "It is in the house." ? "Це у будинку." : "";
      }
    }
  });

  await page.waitForFunction(() => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0);
  // A finished block must never be blanked out again.
  await page.evaluate(() => {
    window.__churn = new Map();
    new MutationObserver((records) => {
      for (const record of records) {
        if (record.target.hasAttribute("data-lwr-pending")) {
          const id = record.target.id;
          window.__churn.set(id, (window.__churn.get(id) || 0) + 1);
        }
      }
    }).observe(document.body, {
      attributes: true,
  ...
  // Scroll back and forth. This is the real trigger: each scroll re-collects
  // whatever the extension still considers unfinished.
  const callsBeforeScrolling = translateCalls.length;
  const runsBefore = await page.evaluate(
    () => window.__learnedWordReplacerDebug.getSnapshot().runId
  );
  // Watch what any pass triggered from here on actually picks up. Counting
  // translator calls proves nothing: an empty translation is cached, so a
  // re-collected block is re-scrambled without calling the translator again.
  await page.evaluate(() => {
    window.__collected = [];
    const debugApi = window.__learnedWordReplacerDebug;
    const snapshot = debugApi.getSnapshot.bind(debugApi);
    window.__watch = setInterval(() => {
      const status = snapshot();
      if (status.runId > window.__runsBefore) {
        window.__collected.push(status.unitsCollected);
      }
    }, 15);
  });
  await page.evaluate((runId) => {
    window.__runsBefore = runId;
  }, runsBefore);
  ...
  const result = await page.evaluate(() => ({
    churn: Object.fromEntries(window.__churn),
    collected: window.__collected,
    innerText: document.getElementById("inner").textContent,
    untranslatableText: document.getElementById("untranslatable").textContent
  }));

  assert(
    Object.keys(result.churn).length === 0,
    `text the reader had already read was blanked again: ${JSON.stringify(result.churn)}`
  );
  // The churn is the visible symptom; the cause is finished blocks being
  // collected all over again. Pin the cause, not just the symptom.
  assert(
    result.collected.every((count) => count === 0),
    `scrolling re-collected finished blocks: ${JSON.stringify(result.collected)}`
  );
  assert(
    translateCalls.length === callsBeforeScrolling,
    `scrolling re-translated finished blocks: ${JSON.stringify(
      translateCalls.slice(callsBeforeScrolling)
    )}`
  );
  assert(
    result.innerText === "It is in the будинку.",
    `the translated block lost its replacement: ${result.innerText}`
  );
  assert(
    result.untranslatableText === "3:30 pm",
    `the untranslatable block was left altered: ${result.untranslatableText}`
  );
  await page.close();
}
...
async function testTargetLanguagePagesAreLeftAloneWhenDisabled(browser) {
  const page = await browser.newPage();
  const source = "The sweater is warm.";
  await page.setContent(
  ...
  await installHarness(page, {
    ...
    translator: {
      ...
      translate: async (text) => {
        ...
        return text === source ? "Светр теплий." : "";
      }
    },
    config: {
      ukrainianLemmas: { светр: ["светр"] }
  ...
  assert(
    !translatedTexts.some((text) => /[Ѐ-ӿ]/u.test(text)),
    `the target-language block was still sent to a translator: ${translatedTexts.join(" | ")}`
  );
  await page.close();
}

// The mirror of structure mode: the page is already Ukrainian, so its own
// sentence is the frame and the unlearned words become English.
function createTargetLanguagePageFixture(extraState = {}) {
  const ukrainian = "Радіо — це технологія зв'язку.";
  const english = "Radio is the technology of communicating.";
  const directions = [];
  ...
  return {
    ...
    english,
    directions,
    html: '<p id="uk-copy">Радіо — це <a href="#">технологія</a> зв\'язку.</p>',
    harness: {
      state: {
        ...createState([
          {
            ...
            source: "radio",
            target: "радіо",
            definition: "Duolingo meanings: radio, wireless",
            enabled: true,
            createdAt: 1
          }
        ]),
      ...
      translator: {
        availability: async () => "available",
        translate: async (text, direction = {}) => {
          directions.push(`${direction.sourceLanguage}:${direction.targetLanguage}`);
          return text === ukrainian ? english : "";
        }
      },
      config: {
        ukrainianLemmas: { "радіо": ["радіо"] },
        reverseHoverDelayMs: 10
      },
      wordAligner: createStructureModeAligner()
    }
  };
...
async function testTargetLanguagePagesSwapUnlearnedWordsIntoEnglish(browser) {
  const page = await browser.newPage();
  const fixture = createTargetLanguagePageFixture();
  await page.setContent(fixture.html);
  await installHarness(page, fixture.harness);
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      text: document.body.innerText,
      known: { text: known?.textContent, original: known?.dataset.learnedWordOriginal },
      scaffold: scaffold.map((span) => `${span.dataset.learnedWordOriginal}→${span.textContent}`),
      linkText: document.querySelector("#uk-copy a")?.textContent,
      replacementCount: window.__learnedWordReplacerDebug.getSnapshot().replacementCount
    };
  });

  assert(
    result.text === "Радіо — is technology communicating.",
    `the target-language page was not read back into English: ${JSON.stringify(result)}`
  );
  assert(
    result.known.text === "Радіо" && result.known.original === "Радіо",
    `the learned word did not stay as the page wrote it: ${JSON.stringify(result.known)}`
  );
  assert(
    result.scaffold.join(",") === "це→is,технологія→technology,зв'язку→communicating",
    `unlearned words were not swapped into English: ${JSON.stringify(result.scaffold)}`
  );
  // Nothing is flattened here, unlike structure mode: the page's own markup
  // survives because each word is replaced inside its own text node.
  assert(
    result.linkText === "technology",
    `the page's inline markup did not survive the pass: ${result.linkText}`
  );
  assert(
    result.replacementCount === 1,
    `English scaffold words were counted as learned-word replacements: ${result.replacementCount}`
  );
  assert(
    fixture.directions.includes("uk:en") && !fixture.directions.includes("en:uk"),
    `the page text was not translated with the target→English direction: ${fixture.directions.join(",")}`
  );
  await page.close();
}

async function testTargetLanguagePageWordsKeepTheirHoverAnswers(browser) {
  const page = await browser.newPage();
  const fixture = createTargetLanguagePageFixture();
  await page.setContent(fixture.html);
  await installHarness(page, fixture.harness);
  ...
  const hoverRows = async (selector) => {
  ...
  // The learned word is shown exactly as the page wrote it, so its hover has to
  // answer with the English side rather than "reveal" the same word again.
  const learnedRows = await hoverRows(
    '.learned-word-replacer-token:not([data-learned-word-match-kind="back-translation"])'
  );
  assert(
    learnedRows.join(",") === "radio,wireless",
    `hovering a learned target-language word did not show its meanings: ${JSON.stringify(learnedRows)}`
  );

  const scaffoldRows = await hoverRows(
    '.learned-word-replacer-token[data-learned-word-match-kind="back-translation"]'
  );
  assert(
    scaffoldRows.join(",") === "це",
    `hovering an English scaffold word did not reveal the page's own word: ${JSON.stringify(scaffoldRows)}`
  );
  await page.close();
}

async function testTargetLanguagePagesRestoreThePageWhenDisabled(browser) {
  const page = await browser.newPage();
  const fixture = createTargetLanguagePageFixture();
  await page.setContent(fixture.html);
  const before = await page.evaluate(() => document.getElementById("uk-copy").innerHTML);
  ...
  await page.evaluate((nextState) => {
    window.__storageChangeListener({ learnedWordReplacerState: { newValue: nextState } }, "local");
  }, { ...fixture.harness.state, targetLanguagePages: false });
  await page.waitForFunction(
    () => document.querySelectorAll(".learned-word-replacer-token").length === 0
  );
  const after = await page.evaluate(() => document.getElementById("uk-copy").innerHTML);

  assert(
    after === before,
    `turning the setting off did not restore the target-language page: ${after}`
  );
  await page.close();
}
...
// A mutation's own pass must handle the block it queued and nothing else — but
// a visible block the budget skipped is not abandoned either: the pass that
// left it behind schedules the follow-up that finishes it. Blocks are only
// collected within the viewport margin, so this finishes the screen, not the
// whole document.
async function testMutationProcessesQueuedBlockFirstThenFinishesTheRest(browser) {
  ...
  // The mutated block is re-read from its own words, not from the half-empty
  // text its replacement spans leave behind, so this is the exact string the
  // translator must be handed.
  const deadline = Date.now() + 15000;
  while (!calls.includes("It is in the house now.") && Date.now() < deadline) {
    await page.waitForTimeout(50);
  }
  await page.waitForFunction(() => document.getElementById("second").innerText.includes("Авто"));

  assert(
    calls.includes("It is in the house now."),
    `the mutated block was never re-read from its own words: ${JSON.stringify(calls)}`
  );
  assert(
    !calls.some((text) => text.includes("in the  now")),
    `the mutated block was translated with its replaced word missing: ${JSON.stringify(calls)}`
  );
  assert(
    calls.indexOf("It is in the house now.") < calls.indexOf("Car."),
    `the mutated block was not translated before the leftover one: ${JSON.stringify(calls)}`
  );
  assert(
    calls.filter((text) => text === "It is in the house.").length === 1,
    `an already processed block was translated again: ${JSON.stringify(calls)}`
  );
  await page.close();
}
...
async function testNavigationAndAsideTextIsTranslated(browser) {
  const page = await browser.newPage();
  await page.setContent(`
    <nav><p>the mother and father</p></nav>
    <aside><p>mother and father</p></aside>
    <main><p>It is in the house and car.</p></main>
    <p hidden>mother and father</p>
    <p contenteditable>mother and father</p>
  `);
  await installHarness(page, {
    state: createState([
    ...
    translator: {
      ...
      translate: async (text) => {
        ...
        if (text === "the mother and father") {
          return "\u043c\u0430\u0442\u0438 \u0456 \u0431\u0430\u0442\u044c\u043a\u043e";
        }
  ...
  // A sidebar or table of contents is reading material like any other text.
  await page.waitForFunction(() => document.querySelector("nav").innerText.includes("\u043c\u0430\u0442\u0438"));
  await page.waitForFunction(
    () => window.__learnedWordReplacerDebug.getSnapshot().finishedAt > 0
  );
  const result = await page.evaluate(() => ({
    ...
    hiddenText: document.querySelector("p[hidden]").textContent,
    editableText: document.querySelector("p[contenteditable]").textContent
  }));

  assert(result.navText.includes("\u043c\u0430\u0442\u0438"), `navigation text was not replaced: ${result.navText}`);
  assert(result.mainText.includes("\u0431\u0443\u0434\u0438\u043d\u043a\u0443"), "main content was not replaced");
  // Regions that are genuinely not readable text stay off limits.
  assert(result.hiddenText === "mother and father", "hidden text should not be replaced");
  assert(result.editableText === "mother and father", "editable text should not be replaced");
  await page.close();
}
...
async function testDuolingoTypeInputFollowsThePageTheme(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      // Duolingo's dark theme paints the page background rather than setting a
      // flag, so that is what the extension reads.
      body: `
        <style>body { background: rgb(19, 31, 36) }</style>
  ...
  const read = () =>
    page.evaluate(() => {
      ...
      const placeholderRule = document.getElementById("learned-word-replacer-duolingo-theme");
      return {
        theme: input.getAttribute("data-lwr-theme"),
        background: style.backgroundColor,
        color: style.color,
        border: style.borderTopColor,
        icon: getComputedStyle(document.getElementById("learned-word-replacer-duolingo-hint-button"))
          .color,
        placeholderCss: placeholderRule ? placeholderRule.textContent : ""
      };
    });

  const dark = await read();
  assert(dark.theme === "dark", `dark page was not detected: ${dark.theme}`);
  assert(
    dark.background === "rgb(32, 47, 54)",
    `type input kept a light background on a dark page: ${dark.background}`
  );
  assert(
    dark.color === "rgb(241, 247, 251)",
    `type input kept dark text on a dark page: ${dark.color}`
  );
  assert(
    dark.icon === "rgb(139, 159, 168)",
    `hint icon kept its light-theme grey on a dark page: ${dark.icon}`
  );
  assert(
    dark.placeholderCss.includes("rgb(139, 159, 168)"),
    `placeholder rule was not written for the dark theme: ${dark.placeholderCss}`
  );

  // The row builds focused, and border-color is transitioned, so the resting
  // dark border only shows once focus leaves and the 0.15s ease lands.
  const settledBorder = () =>
    page.waitForFunction(
      (expected) =>
        getComputedStyle(document.getElementById("learned-word-replacer-duolingo-type-input"))
          .borderTopColor === expected,
      "rgb(55, 70, 79)"
    );
  await page.evaluate(() => {
    document.getElementById("learned-word-replacer-duolingo-type-input").blur();
  });
  await settledBorder().catch(() => {
    throw new Error("type input never settled on the dark resting border");
  });

  // A live theme flip has to repaint the row that is already on screen, not
  // wait for the next challenge to rebuild it.
  await page.evaluate(() => {
    document.body.style.background = "rgb(255, 255, 255)";
  });
  await page.waitForFunction(
    () =>
      document.getElementById("learned-word-replacer-duolingo-type-input").getAttribute(
        "data-lwr-theme"
      ) === "light"
  );
  ...
  assert(
    ...
    `type input stayed dark after the page went light: ${light.background}`
  );
  assert(
    light.color === "rgb(60, 60, 60)",
    `type input kept light text after the page went light: ${light.color}`
  );
  assert(
    light.placeholderCss.includes("rgb(175, 175, 175)"),
    `placeholder rule did not follow the flip back to light: ${light.placeholderCss}`
  );

  await page.close();
}

async function testDuolingoWordsPageInputsFollowTheDarkTheme(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/practice-hub/words", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: `
        <style>body { background: rgb(19, 31, 36); color: rgb(241, 247, 251) }</style>
  ...
  await page.waitForSelector("[data-lwr-manual-source]", { state: "attached" });
  // Both panels build up front but stay hidden behind their tab; open them so
  // the assertions read what the user would actually be looking at.
  await page.click("button[data-lwr-words-tab='manual']");
  await page.click("button[data-lwr-words-tab='flashcards']");

  const styles = await page.evaluate(() =>
    [...document.querySelectorAll("[data-lwr-panel-input]")].map((input) => {
      const style = getComputedStyle(input);
      return {
        background: style.backgroundColor,
        color: style.color,
  ...
  assert(styles.length >= 3, `expected the manual and flashcard inputs, saw ${styles.length}`);
  for (const style of styles) {
    assert(
      style.background === "rgb(32, 47, 54)",
      `a Words-page input kept a light background on a dark page: ${style.background}`
    );
    assert(
      style.color === "rgb(241, 247, 251)",
      `a Words-page input kept dark text on a dark page: ${style.color}`
    );
    assert(
      style.border === "rgb(55, 70, 79)",
      `a Words-page input kept its light border on a dark page: ${style.border}`
    );
  }
...
async function testDuolingoTypeHintShowsNextLettersAndDeadEnds(browser) {
  ...
  // The first three presses are shape rungs (see the blur-ladder test below);
  // a fourth walks the ladder on to the letter reveal these assertions cover.
  const probe = (typed) =>
    page.evaluate((value) => {
      const input = document.getElementById("learned-word-replacer-duolingo-type-input");
      input.value = value;
...
// Tab is a "give me a bit more" key. The first three presses cue the word's
// shape behind a sharpening blur — length and letter runs, nothing spelled
// out — and only then does it start handing over letters. The rung belongs to
// the typed buffer: editing it (or placing the word) starts the ladder over.
async function testDuolingoTypeHintLadderBlursBeforeLetters(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: `
        <div data-test="word-bank">
          <button data-test="challenge-tap-token">привіт</button>
          <button data-test="challenge-tap-token">пробач</button>
          <button data-test="challenge-tap-token">мене</button>
          <button data-test="challenge-tap-token">мене звуть</button>
        </div>
      `
  ...
  const rungsForPresses = (typed, presses) =>
    page.evaluate(
      ({ value, count }) => {
        const input = document.getElementById("learned-word-replacer-duolingo-type-input");
        input.value = value;
        ...
        for (let i = 0; i < count; i += 1) {
          input.dispatchEvent(new KeyboardEvent("keydown", { key: "Tab", bubbles: true }));
          const shape = badge.querySelector("span[aria-hidden='true']");
          rungs.push({
            text: badge.textContent,
            blur: shape ? shape.style.filter : "",
            shape: shape ? shape.textContent : ""
          });
        }
        return rungs;
      },
      { value: typed, count: presses }
    );

  const ladder = await rungsForPresses("при", 7);
  assert(
    JSON.stringify(ladder.slice(0, 3).map((rung) => rung.blur)) ===
      JSON.stringify(["blur(10px)", "blur(6px)", "blur(3.5px)"]),
    `shape rungs did not sharpen: ${JSON.stringify(ladder.slice(0, 3))}`
  );
  assert(
    ladder.slice(0, 3).every((rung) => rung.shape === "привіт" && rung.text === "shape:привіт"),
    `shape rungs did not blur the candidate word: ${JSON.stringify(ladder.slice(0, 3))}`
  );
  assert(
    JSON.stringify(ladder.slice(3).map((rung) => rung.text)) ===
      JSON.stringify(["next: в", "next: ві", "next: віт", "next: віт"]),
    `ladder did not fall through to one more letter per press: ${JSON.stringify(
      ladder.slice(3).map((rung) => rung.text)
    )}`
  );
  assert(
    ladder.slice(3).every((rung) => rung.blur === ""),
    "letter rungs still rendered a blurred word"
  );

  // Two candidates share the prefix, so the shape rung shows both silhouettes
  // and the letter rungs keep both branches rather than picking one.
  const branching = await rungsForPresses("пр", 5);
  assert(
    branching[0].shape === "привіт · пробач",
    `shape rung dropped a candidate: ${JSON.stringify(branching[0])}`
  );
  assert(
    JSON.stringify(branching.slice(3).map((rung) => rung.text)) ===
      JSON.stringify(["next: и / о", "next: ив / об"]),
    `deeper hint dropped a branch: ${JSON.stringify(branching.slice(3))}`
  );

  // A completed word keeps its "space places it" verdict on every rung, and
  // the letters walk into the multi-word continuation a space at a time.
  const completed = await rungsForPresses("мене", 5);
  assert(
    completed[0].text === "space places it · shape:мене звуть",
    `completed word lost its verdict on the shape rung: ${JSON.stringify(completed[0])}`
  );
  assert(
    JSON.stringify(completed.slice(3).map((rung) => rung.text)) ===
      JSON.stringify([
        "space places it · or continue: ␣",
        "space places it · or continue: ␣з"
      ]),
    `completed-word hint did not deepen: ${JSON.stringify(completed.slice(3))}`
  );

  // A word the buffer already spells out has no shape left to cue, so the
  // ladder skips straight to the verdict.
  const spelledOut = await rungsForPresses("привіт", 1);
  assert(
    spelledOut[0].text === "space places it" && spelledOut[0].blur === "",
    `fully typed word still got a shape rung: ${JSON.stringify(spelledOut[0])}`
  );

  // A dead end is a dead end on the first press: nothing to blur, nothing to
  // reveal, just the red "backspace" verdict.
  const deadEnd = await rungsForPresses("яб", 1);
  assert(
    deadEnd[0].text === "✗ no bank word matches — backspace" && deadEnd[0].blur === "",
    `dead end was given a shape rung: ${JSON.stringify(deadEnd[0])}`
  );

  // Editing the buffer resets the ladder: retyping must start back at the
  // coarsest blur, not resume where the last word left off.
  const afterEdit = await page.evaluate(() => {
    const input = document.getElementById("learned-word-replacer-duolingo-type-input");
    const badge = document.getElementById("learned-word-replacer-duolingo-hint-badge");
    const blurNow = () => {
      const shape = badge.querySelector("span[aria-hidden='true']");
      return shape ? shape.style.filter : "";
    };
    input.value = "при";
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    input.dispatchEvent(new KeyboardEvent("keydown", { key: "Tab", bubbles: true }));
    ...
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    input.dispatchEvent(new KeyboardEvent("keydown", { key: "Tab", bubbles: true }));
    return { deep, afterTyping: blurNow() };
  });
  assert(
    afterEdit.deep === "blur(6px)" && afterEdit.afterTyping === "blur(10px)",
    `typing did not reset the hint ladder: ${JSON.stringify(afterEdit)}`
  );

  // The lightbulb button is the mouse path up the same ladder.
  const viaButton = await page.evaluate(() => {
    ...
    input.value = "при";
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    const button = document.getElementById("learned-word-replacer-duolingo-hint-button");
    button.click();
    const first = blurNow();
    button.click();
    return { first, second: blurNow() };
  });
  assert(
    viaButton.first === "blur(10px)" && viaButton.second === "blur(6px)",
    `hint button did not climb the ladder like Tab: ${JSON.stringify(viaButton)}`
  );
...
async function testDuolingoListenMatchHidesWordsAndTypesPairs(browser) {
  ...
  const afterPair = await page.evaluate(() => {
    const input = document.getElementById("learned-word-replacer-duolingo-type-input");
    // Past the three shape rungs to the letter reveal.
    for (let press = 0; press < 4; press += 1) {
      input.dispatchEvent(new KeyboardEvent("keydown", { key: "Tab", bubbles: true }));
    }
    const badge = document.getElementById("learned-word-replacer-duolingo-hint-badge");
...
function wordBankLessonBody() {
  // Mirrors the live word-bank DOM (2026-08-07): each token sits in its own
  // slot wrapper, and the word itself is in a challenge-tap-token-text span.
  // "Швидко" carries the capital that gives away the first word.
  const slot = (word, placed) =>
    `<div class="_1uV0Q" style="height: 40px; margin: 4px"><span><button data-test="${word}-challenge-tap-token" lang="uk" aria-disabled="${placed ? "true" : "false"}" style="height: 40px; padding: 0 12px"><span></span><span><span data-test="challenge-tap-token-text">${word}</span></span></button></span></div>`;
  // Width and row height are set so the bank actually wraps: the decoys must
  // be held inside the height Duolingo's own words needed, not push the
  // challenge up the screen.
  return `
    <div data-test="word-bank" style="display: flex; flex-wrap: wrap; width: 380px; align-content: flex-start">
      ${slot("Швидко")}${slot("ідіть")}${slot("сюди", true)}${slot("моя")}${slot("песимістка")}${slot("мій")}
    </div>
    <script>
      window.__tokenClicks = [];
      document.querySelectorAll("button[data-test*='challenge-tap-token']").forEach((button) => {
        button.addEventListener("click", () => {
          window.__tokenClicks.push(button.textContent.trim());
        });
      });
    </script>
  `;
...
function editDistance(a, b) {
  ...
  const first = [...a];
  const second = [...b];
  let previous = Array.from({ length: columns }, (unused, index) => index);
...
// The word bank answers two questions it was never asked: which word starts
// the sentence (it is the capitalised one) and how each word is spelled (every
// option is spelled right). Lowercasing and near-miss decoys take both away.
async function testDuolingoWordBankLowercasesAndPlantsDecoys(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({ contentType: "text/html; charset=utf-8", body: wordBankLessonBody() })
  );
  await page.goto("https://www.duolingo.com/lesson");
  const lessonState = {
    ...createState([]),
  ...
  await page.waitForFunction(() => document.querySelectorAll("[data-lwr-decoy]").length > 0, null, {
    timeout: 5000
  });
  ...
  const readBank = () =>
    page.evaluate(() => {
      ...
      return {
        real: [...bank.querySelectorAll("button[data-test*='challenge-tap-token']")].map((button) =>
          button.textContent.trim()
        ),
        decoys: [...bank.querySelectorAll(`button[data-lwr-decoy]`)].map((button) =>
          button.textContent.trim()
        ),
        // A decoy that only ever sits at the end would be named by its place.
        decoySlots: [...bank.children]
          .map((slot, index) => (slot.hasAttribute("data-lwr-decoy") ? index : -1))
  ...
  assert(
    bank.real.join(",") === "швидко,ідіть,сюди,моя,песимістка,мій",
    `word bank was not lowercased: ${bank.real.join(",")}`
  );
  // Words of four letters or more are the ones worth being unsure about, and
  // every one of them has to draw a decoy — the hardest word going unpaired is
  // exactly what makes the bank readable again.
  const worthFaking = bank.real.filter(
    (word) => [...word].filter((letter) => /\p{L}/u.test(letter)).length >= 4
  );
  const nearMiss = (word, decoy) =>
    [...word].length === [...decoy].length && editDistance(word, decoy) <= 2;

  assert(
    bank.decoys.length >= worthFaking.length && bank.decoys.length <= 12,
    `expected a decoy for each of ${worthFaking.length} words, got ${JSON.stringify(bank.decoys)}`
  );
  for (const decoy of bank.decoys) {
    assert(!bank.real.includes(decoy), `decoy "${decoy}" is a real word in the bank`);
    // Same length as its source: a doubled or dropped letter changes a word's
    // shape, and the odd one out is then spotted without being read.
    assert(
      bank.real.some((word) => nearMiss(word, decoy)),
      `decoy "${decoy}" is not a same-length near miss of any bank word`
    );
  }
  for (const word of worthFaking) {
    assert(
      bank.decoys.some((decoy) => nearMiss(word, decoy)),
      `"${word}" drew no decoy: ${JSON.stringify(bank.decoys)}`
    );
  }

  const hooks = await page.evaluate(() => ({
    realTokens: document.querySelectorAll("button[data-test*='challenge-tap-token']").length,
    decoyDataTest: [...document.querySelectorAll("button[data-lwr-decoy]")].map((button) =>
      button.getAttribute("data-test")
    )
  }));
  assert(
    hooks.realTokens === 6 && hooks.decoyDataTest.every((value) => value === null),
    "decoys still read as real tokens to everything selecting on Duolingo's hooks"
  );

  // Duolingo re-renders the challenge constantly; the decoys must hold still
  // rather than reshuffling under the pointer.
  await page.evaluate(() => document.body.appendChild(document.createElement("div")));
  await page.waitForTimeout(150);
  const settled = await readBank();
  assert(
    settled.decoys.join(",") === bank.decoys.join(",") &&
      settled.decoySlots.join(",") === bank.decoySlots.join(","),
    `decoys moved on re-render: ${JSON.stringify(bank)} then ${JSON.stringify(settled)}`
  );

  const scroll = await page.evaluate(() => {
    const bank = document.querySelector("[data-test='word-bank']");
    return {
      capped: bank.style.maxHeight,
      height: Math.round(bank.getBoundingClientRect().height),
      content: bank.scrollHeight,
      scrolls: bank.scrollHeight > bank.clientHeight + 1,
      marked: bank.getAttribute("data-lwr-bank-scroll"),
      styled: Boolean(document.getElementById("learned-word-replacer-duolingo-bank-scroll-style"))
    };
  });
  assert(scroll.marked === "1" && scroll.styled, "the bank was not turned into a scroller");
  assert(
    scroll.capped === `${scroll.height}px` && scroll.height === 96,
    `the bank did not keep the two rows it started with: ${JSON.stringify(scroll)}`
  );
  assert(
    scroll.scrolls,
    `the decoys did not end up behind a scrollbar: ${JSON.stringify(scroll)}`
  );

  const clicked = await page.evaluate(() => {
    const decoy = document.querySelector("button[data-lwr-decoy]");
    decoy.click();
    return { clicks: [...window.__tokenClicks], colour: decoy.style.color };
  });
  assert(clicked.clicks.length === 0, `clicking a decoy placed a word: ${clicked.clicks}`);
  assert(clicked.colour === "rgb(234, 43, 43)", `a clicked decoy did not refuse: ${clicked.colour}`);

  const typed = await page.evaluate((decoyWord) => {
    const input = document.getElementById("learned-word-replacer-duolingo-type-input");
    input.value = decoyWord;
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    input.dispatchEvent(new KeyboardEvent("keydown", { key: " ", bubbles: true }));
    const afterDecoy = { clicks: [...window.__tokenClicks], border: input.style.borderColor };

    input.value = "песимістка";
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    input.dispatchEvent(new KeyboardEvent("keydown", { key: " ", bubbles: true }));
    return { afterDecoy, clicks: [...window.__tokenClicks] };
  }, bank.decoys[0]);
  assert(
    typed.afterDecoy.clicks.length === 0 && typed.afterDecoy.border === "rgb(234, 43, 43)",
    `typing a decoy spelling was accepted: ${JSON.stringify(typed.afterDecoy)}`
  );
  assert(
    typed.clicks.join(",") === "песимістка",
    `typing the real word stopped working: ${JSON.stringify(typed.clicks)}`
  );

  // Switching both off hands the page back as Duolingo wrote it.
  await page.evaluate(
    (stored) => window.__storageChangeListener({ learnedWordReplacerState: { newValue: stored } }, "local"),
    { ...lessonState, duolingoLowercaseBank: false, duolingoDecoyWords: false }
  );
  await page.waitForFunction(
    () => document.querySelectorAll("[data-lwr-decoy]").length === 0,
    null,
    { timeout: 5000 }
  );
  const restored = await page.evaluate(() =>
    [...document.querySelectorAll("[data-test='challenge-tap-token-text']")].map(
      (span) => span.textContent
    )
  );
  assert(
    restored.join(",") === "Швидко,ідіть,сюди,моя,песимістка,мій",
    `the capital was not handed back: ${restored.join(",")}`
  );
  const released = await page.evaluate(() => {
    const bank = document.querySelector("[data-test='word-bank']");
    return { capped: bank.style.maxHeight, marked: bank.getAttribute("data-lwr-bank-scroll") };
  });
  assert(
    !released.capped && released.marked === null,
    `the bank was left as a scroller: ${JSON.stringify(released)}`
  );

  await page.close();
}

function matchPairsLessonBody() {
  // Mirrors the live challenge-match DOM (2026-08-07): ten numbered word
  // cards in two columns, each card's data-test its own word — the pairing is
  // nowhere in the DOM, unlike listen-match.
  const card = (word, badge) =>
    `<button data-test="${word}-challenge-tap-token" aria-disabled="false"><span>${badge}</span><span data-test="challenge-tap-token-text">${word}</span></button>`;
  return `
    <div data-test="challenge challenge-match">
      <h1 data-test="challenge-header"><span>Select the matching pairs</span></h1>
      <div id="pairs" style="display: flex; gap: 40px">
        <div style="display: flex; flex-direction: column; width: 200px">
          ${card("November", 1)}${card("July", 2)}${card("boots", 3)}${card("coat", 4)}${card("May", 5)}
        </div>
        <div style="display: flex; flex-direction: column; width: 200px">
          ${card("чоботи", 6)}${card("листопад", 7)}${card("травень", 8)}${card("липень", 9)}${card("пальто", 0)}
        </div>
      </div>
    </div>
    <script>
      window.__cardClicks = [];
      document.querySelectorAll("button[data-test*='challenge-tap-token']").forEach((card) => {
        card.addEventListener("click", () => {
          window.__cardClicks.push(
            card.querySelector("[data-test='challenge-tap-token-text']").textContent
          );
        });
      });
    </script>
...
// Matching-pairs challenges show words on both sides, so the listen-match rule
// ("hide every card's text") would leave nothing to read. Only the column in
// the language being learned is hidden and typed; the other side is picked by
// its number badge, and the tenth card is numbered 0.
async function testDuolingoMatchPairsHidesAndTypesTheTargetColumn(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({ contentType: "text/html; charset=utf-8", body: matchPairsLessonBody() })
  );
  await page.goto("https://www.duolingo.com/lesson");
  await installHarness(page, {
    state: { ...createState([]), duolingoTypeAnswers: true },
  ...
  const readVisibility = () =>
    page.evaluate(() =>
      [...document.querySelectorAll("[data-test='challenge-tap-token-text']")].map((span) => [
        span.textContent,
  ...
  const setup = await page.evaluate(() => {
    ...
    return {
      placeholder: input.placeholder,
      insertedBeforeCards: wrap.nextElementSibling === document.getElementById("pairs")
    };
  });
  assert(
    setup.placeholder === "Press a number to pick a word, type its pair, then space",
    `wrong pairs placeholder: ${setup.placeholder}`
  );
  assert(setup.insertedBeforeCards, "input row was not inserted before the card grid");

  const hidden = await readVisibility();
  const hiddenWords = hidden.filter(([, visibility]) => visibility === "hidden").map(([word]) => word);
  assert(
    hiddenWords.join(",") === "чоботи,листопад,травень,липень,пальто",
    `wrong column hidden: ${JSON.stringify(hidden)}`
  );
  ...
  const typed = await page.evaluate(() => {
    ...
    // Pick the English card by its number, then type the word it pairs with.
    press("1");
    input.value = "листопад";
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    press(" ");
    const afterPair = [...window.__cardClicks];
    const bufferAfterPair = input.value;

    // The tenth card is numbered 0.
    press("0");

    // English words are on the visible side: they are read, not recalled, so
    // typing one must not place a card.
    input.value = "boots";
    input.dispatchEvent(new InputEvent("input", { bubbles: true }));
    press(" ");
    return {
  ...
  assert(
    typed.afterPair.join(",") === "November,листопад",
    `number pick then typed word did not pair: ${JSON.stringify(typed.afterPair)}`
  );
  assert(typed.bufferAfterPair === "", "buffer was not cleared after placing the word");
  assert(
    typed.clicks.join(",") === "November,листопад,пальто",
    `wrong cards clicked: ${JSON.stringify(typed.clicks)}`
  );
  assert(
    typed.border === "rgb(234, 43, 43)",
    "typing a word from the visible column did not dead-end"
  );

  await page.click("#learned-word-replacer-duolingo-bank-toggle");
  const revealed = await readVisibility();
  assert(
    revealed.every(([, visibility]) => visibility !== "hidden"),
    `the eye toggle did not reveal the hidden column: ${JSON.stringify(revealed)}`
  );

  await page.close();
}

// A Latin-script target language leaves the script test blind, so the column
// is chosen by geometry instead: Duolingo puts the language being learned on
// the right.
async function testDuolingoMatchPairsFallsBackToTheRightColumn(browser) {
  const page = await browser.newPage();
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: matchPairsLessonBody()
        .replace(/чоботи/g, "botas")
        .replace(/листопад/g, "noviembre")
        .replace(/травень/g, "mayo")
        .replace(/липень/g, "julio")
        .replace(/пальто/g, "abrigo")
    })
  );
  await page.goto("https://www.duolingo.com/lesson");
  await installHarness(page, {
    state: {
      ...
      duolingoTypeAnswers: true,
      currentProfileId: "es-test",
      profiles: [{ id: "es-test", name: "Spanish Test", languageCode: "es", entries: [] }]
    },
    translator: {
      availability: async () => "available",
      translate: async (text) => text
  ...
  assert(
    hidden.join(",") === "botas,noviembre,mayo,julio,abrigo",
    `geometry fallback hid the wrong column: ${JSON.stringify(hidden)}`
  );
  ...
  assert(
    clicks.join(",") === "July,julio",
    `typing the hidden Spanish word did not pair it: ${JSON.stringify(clicks)}`
  );

  await page.close();
}

// The phrase an exercise puts in front of you is the one worth looking up
// afterwards, so the prompt carries a copy button. The typing input owns focus
// for the whole challenge, which is why ⌘/Ctrl+C with nothing selected copies
// the phrase instead of coming back empty.
async function testDuolingoLessonPhraseIsCopyable(browser) {
  const page = await browser.newPage();
  // Mirrors the live challenge-translate DOM (2026-08-06): the prompt has no
  // data-test hook of its own, only a lang attribute and hint tokens, and its
  // class names are hashed per build.
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: `
        <div data-test="challenge challenge-translate">
          <h1 data-test="challenge-header"><span>Write this in English</span></h1>
          <div class="_2w0y3"><div class="_20npu" dir="ltr" lang="uk"><span><span data-test="hint-token">Ти</span> <span data-test="hint-token">сильний</span>, <span data-test="hint-token">а</span> <span data-test="hint-token">він</span> <span data-test="hint-token">слабкий</span>!</span></div></div>
          <div data-test="word-bank">
            <button data-test="You-challenge-tap-token" lang="en"><span data-test="challenge-tap-token-text">You</span></button>
            <button data-test="are-challenge-tap-token" lang="en"><span data-test="challenge-tap-token-text">are</span></button>
            <button data-test="strong-challenge-tap-token" lang="en"><span data-test="challenge-tap-token-text">strong</span></button>
          </div>
        </div>
        <script>
          window.__promptClicks = 0;
          document
            .querySelector("[lang='uk']")
            .addEventListener("click", () => {
              window.__promptClicks += 1;
            });
        </script>
      `
    })
  ...
  await page.goto("https://www.duolingo.com/lesson");
  await installHarness(page, {
    state: { ...createState([]), duolingoTypeAnswers: true, duolingoCopyPhrase: true },
    translator: {
      availability: async () => "available",
      translate: async (text) => text
    }
  ...
  await page.waitForSelector("#learned-word-replacer-duolingo-copy-button");

  const placement = await page.evaluate(() => {
    const button = document.getElementById("learned-word-replacer-duolingo-copy-button");
    ...
    return {
      parentLang: button.parentElement.getAttribute("lang"),
      insideBank: Boolean(button.closest("[data-test='word-bank']")),
      count: document.querySelectorAll("#learned-word-replacer-duolingo-copy-button").length,
      title: button.title
    };
  });
  assert(
    placement.parentLang === "uk",
    `copy button was not attached to the prompt: ${placement.parentLang}`
  );
  assert(!placement.insideBank, "copy button was attached to the word bank");
  assert(placement.count === 1, `expected one copy button, found ${placement.count}`);
  assert(placement.title === "Copy this phrase", `wrong copy button title: ${placement.title}`);

  await page.click("#learned-word-replacer-duolingo-copy-button");
  const clicked = await page.evaluate(() => ({
    writes: [...window.__clipboardWrites],
    promptClicks: window.__promptClicks,
    state: document
      .getElementById("learned-word-replacer-duolingo-copy-button")
      .getAttribute("data-copy-state")
  }));
  assert(
    clicked.writes.length === 1 && clicked.writes[0] === "Ти сильний, а він слабкий!",
    `wrong copied phrase: ${JSON.stringify(clicked.writes)}`
  );
  assert(clicked.state === "copied", `copy was not confirmed on the button: ${clicked.state}`);
  assert(
    clicked.promptClicks === 0,
    "the copy click also reached the prompt, which opens Duolingo's hint popover"
  );
  ...
  const typed = await page.evaluate(() => {
    ...
    const afterShortcut = window.__clipboardWrites.length;

    // A real selection is the user asking for their own text, not the phrase.
    input.value = "strong";
    input.setSelectionRange(0, 6);
    input.dispatchEvent(new KeyboardEvent("keydown", { key: "c", metaKey: true, bubbles: true }));
    return { afterShortcut, afterSelection: window.__clipboardWrites.length };
  });
  assert(
    typed.afterShortcut === 2,
    `⌘C in the typing input did not copy the phrase: ${typed.afterShortcut} writes`
  );
  assert(
    typed.afterSelection === 2,
    "⌘C overrode a real selection in the typing input"
  );

  // Chrome refuses navigator.clipboard when the document is not focused, which
  // happens right after a challenge transition; the scratch-textarea fallback
  // still copies, and hands focus back to the typing input.
  const fallback = await page.evaluate(async () => {
    window.__execCopies = [];
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
      get: () => ({ writeText: () => Promise.reject(new Error("document is not focused")) })
    });
    document.execCommand = () => {
      window.__execCopies.push({
  ...
  assert(
    fallback.copies.length === 1 &&
      fallback.copies[0].tag === "TEXTAREA" &&
      fallback.copies[0].value === "Ти сильний, а він слабкий!",
    `clipboard fallback did not copy the phrase: ${JSON.stringify(fallback.copies)}`
  );
  assert(
    fallback.focusedId === "learned-word-replacer-duolingo-type-input",
    `fallback left focus on ${fallback.focusedId || "nothing"}`
  );
  assert(fallback.state === "copied", `fallback copy was not confirmed: ${fallback.state}`);

  await page.close();
}

// Gap-fill challenges tag one span per word instead of tagging the sentence,
// and they hint words with empty positioned overlays. Reading the first tagged
// element copied a single word ("Ці") and parked the button mid-sentence.
async function testDuolingoGapFillPhraseCopiesTheWholeSentence(browser) {
  const page = await browser.newPage();
  // Mirrors the live challenge-gapFill DOM (2026-08-07): per-word lang spans,
  // each hinted word split into per-letter spans under an empty hint-token
  // overlay, an empty inline-flex box for the blank, and answer choices that
  // carry lang too.
  await page.route("https://www.duolingo.com/lesson", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: `
        <div data-test="challenge challenge-gapFill">
          <h1 data-test="challenge-header"><span>Fill in the blank</span></h1>
          <div class="_3gSoe"><div><span lang="uk"><span aria-hidden="true">Ц</span><span aria-hidden="true">і</span><div data-test="hint-token" aria-label="Ці" style="position: absolute; width: 16px; height: 26px"></div></span><span lang="uk"> </span><span lang="uk">туфлі</span><span lang="uk"> </span><span lang="uk">старі</span><span lang="uk">,</span><span lang="uk"> </span><span lang="uk">але</span><span lang="uk"> </span><span lang="uk">дуже</span><span lang="uk"> </span><span style="display: inline-flex; width: 70px; height: 26px"></span><span lang="uk">.</span></div></div>
          <div><div id="choices">
            <div data-test="challenge-choice"><span>1</span><span data-test="challenge-judge-text" lang="uk">зручне</span></div>
            <div data-test="challenge-choice"><span>2</span><span data-test="challenge-judge-text" lang="uk">зручний</span></div>
            <div data-test="challenge-choice"><span>3</span><span data-test="challenge-judge-text" lang="uk">зручні</span></div>
          </div></div>
        </div>
      `
    })
  ...
  await page.goto("https://www.duolingo.com/lesson");
  await installHarness(page, {
    state: { ...createState([]), duolingoCopyPhrase: true },
    translator: {
      availability: async () => "available",
      translate: async (text) => text
    }
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      parentHoldsWholeSentence: button.parentElement.querySelectorAll("[lang]").length === 12,
      lastChild: button.parentElement.lastElementChild === button
    };
  });
  assert(
    result.parentHoldsWholeSentence,
    "copy button was attached to a single word instead of the sentence"
  );
  assert(result.lastChild, "copy button was not placed at the end of the sentence");

  await page.click("#learned-word-replacer-duolingo-copy-button");
  const copied = await page.evaluate(() => window.__clipboardWrites);
  assert(
    copied.length === 1 && copied[0] === "Ці туфлі старі, але дуже ___.",
    `wrong copied gap-fill phrase: ${JSON.stringify(copied)}`
  );
...
async function testDuolingoSettingsPageShowsExtensionPanel(browser) {
  ...
  assert(panelState.rowCount === 14, `expected 14 setting rows, got ${panelState.rowCount}`);
  assert(
  ...
  const extraSections = await page.evaluate(() => {
    ...
    return {
      ...
      ungroupedRows: [...panel.querySelectorAll("input[data-lwr-setting]")]
        .filter((checkbox) => !checkbox.closest("section")?.querySelector("h2"))
        .map((checkbox) => checkbox.getAttribute("data-lwr-setting"))
    };
  });
  assert(
  ...
  assert(
    extraSections.headings.join(",") ===
      "Translation,On the page,Hovering a word,Duolingo lessons,Do not translate,Vocabulary files",
    `panel is missing its sections: ${extraSections.headings.join(",")}`
  );
  assert(
    !extraSections.ungroupedRows.length,
    `settings rows sit outside a headed section: ${extraSections.ungroupedRows.join(",")}`
  );
  for (const label of [
    "Import file",
    "Import manual file",
  ...
  // A setting that overrides others greys them out rather than leaving toggles
  // on screen that quietly do nothing.
  await page.evaluate(() => {
    window.__storageChangeListener(
      {
  ...
  await page.waitForFunction(
    () =>
      document.querySelector("input[data-lwr-setting='structureMode']")?.disabled === true
  );
  const superseded = await page.evaluate(() => {
    const row = (key) => {
      const checkbox = document.querySelector(`input[data-lwr-setting='${key}']`);
      return {
        disabled: checkbox.disabled,
        dimmed: checkbox.closest("label").style.opacity === "0.45"
      };
    };
    return {
      structureMode: row("structureMode"),
      targetLanguagePages: row("targetLanguagePages"),
      enabled: row("enabled")
    };
  });
  assert(
    superseded.structureMode.disabled &&
      superseded.structureMode.dimmed &&
      superseded.targetLanguagePages.disabled &&
      superseded.targetLanguagePages.dimmed,
    `full translation did not grey out the settings it overrides: ${JSON.stringify(superseded)}`
  );
  assert(
    !superseded.enabled.disabled && !superseded.enabled.dimmed,
    "an unrelated setting was greyed out too"
  );
...
(async () => {
  ...
  try {
    ...
    await testEnglishHintsResolveWithOneAlignmentCheck(browser);
    await testAlignmentContradictionDropsCoincidentalHint(browser);
    await testStructureModeRebuildsSentencesInTargetOrder(browser);
    await testStructureModeGlossesEnglishPossessiveAsOf(browser);
    await testStructureModeKeepsEnglishOwnCapitals(browser);
    await testLearnedWordSpendsItsEnglishOnce(browser);
    ...
    await testFullTranslationPaintsTheWholeSentenceInTargetLanguage(browser);
    await testFullTranslationPaintsWithoutTheAligner(browser);
    await testFullTranslationLeavesTargetLanguagePagesAlone(browser);
    await testFullTranslationSettlesInsteadOfReprocessingItself(browser);
    await testFullTranslationKeepsLinksAroundTheirTranslatedWords(browser);
    await testFullTranslationKeepsPunctuationInsideOneLinkCopy(browser);
    await testFullTranslationRebuildsCustomElementWrappers(browser);
    await testFullTranslationCarriesIconsThroughTheRebuild(browser);
    await testFullTranslationTrustsTransliteratedNames(browser);
    await testRevealedTextIsTranslatedAfterAClick(browser);
    await testTextScrolledIntoViewInsideAPaneIsTranslated(browser);
    await testFullTranslationKeepsSentencesWithOneUnlistedWord(browser);
    await testTranslationContinuesPastOnePassBudget(browser);
    ...
    await testUkrainianApostropheIsNotAWordBoundary(browser);
    await testUkrainianHyphenCompoundIsNotSplit(browser);
    ...
    await testCloakHandsOffToThePendingHide(browser);
    await testCloakLiftsImmediatelyOnExcludedPages(browser);
    await testCloakSurvivesThePageRewritingTheRootClass(browser);
    await testCloakLiftsItselfWithoutTheContentScript(browser);
    await testPendingBlocksStayHiddenUntilTheirTranslationLands(browser);
    await testHiddenTextIsRevealedWhenTranslationFails(browser);
    await testScrollingIntoNewTextNeverBlanksIt(browser);
    await testAlreadyReadBlocksNeverChurnAgain(browser);
    await testBlocksRebuiltWithoutVisibleMarksAreStillMarked(browser);
    ...
    await testTargetLanguagePagesAreLeftAloneWhenDisabled(browser);
    await testTargetLanguagePagesSwapUnlearnedWordsIntoEnglish(browser);
    await testTargetLanguagePageWordsKeepTheirHoverAnswers(browser);
    await testTargetLanguagePagesRestoreThePageWhenDisabled(browser);
    ...
    await testMutationProcessesQueuedBlockFirstThenFinishesTheRest(browser);
    await testScrollDoesNotRewriteStructuralContainerSiblings(browser);
    await testNavigationAndAsideTextIsTranslated(browser);
    ...
    await testDuolingoTypeHintLadderBlursBeforeLetters(browser);
    await testDuolingoTypeInputFollowsThePageTheme(browser);
    await testDuolingoWordsPageInputsFollowTheDarkTheme(browser);
    await testDuolingoListenMatchHidesWordsAndTypesPairs(browser);
    await testDuolingoAssistHidesChoicesAndTypesAnswer(browser);
    await testDuolingoWordBankLowercasesAndPlantsDecoys(browser);
    await testDuolingoMatchPairsHidesAndTypesTheTargetColumn(browser);
    await testDuolingoMatchPairsFallsBackToTheRightColumn(browser);
    await testDuolingoLessonPhraseIsCopyable(browser);
    await testDuolingoGapFillPhraseCopiesTheWholeSentence(browser);
    ...
    testCloakRunsBeforeThePagePaints();
